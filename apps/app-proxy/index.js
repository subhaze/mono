...
appDomain.set(
	`faulty-technology.${BASE_DOMAIN}:${PORT}`,
	`http://localhost:3000`
);
appDomain.set(
	`asocial-network.${BASE_DOMAIN}:${PORT}`,
	`http://localhost:3001`
);
...
Domain unknown: ${req.headers.host}

${Array.from(appDomain).map(
	(a) => `<div><a href="https://${a[0]}">${a[0]} -> ${a[1]}</a></div>`
).join('')}
